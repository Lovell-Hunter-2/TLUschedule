...
export async function syncToGoogleCalendar(subjects: Subject[]) {
  try {
    ...
    if (!token) throw new Error("Không thể lấy quyền truy cập Google Calendar");
    ...
    for (const subject of subjects) {
      ...
      for (const dayOfWeek of subject.daysOfWeek) {
        ...
        const event = {
          ...
          start: { dateTime: eventStartTime.toISOString(), timeZone: 'Asia/Ho_Chi_Minh' },
          end: { dateTime: eventEndTime.toISOString(), timeZone: 'Asia/Ho_Chi_Minh' },
          recurrence: [rrule],
